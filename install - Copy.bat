...
GOTO START_INSTALL_AUTOBOT_LIB
...
:installOptimusPackage
	...
	if not exist tmp\optimus_package.zip (
		...
		powershell wget https://github.com/ray-oh/Optimus-Installation/raw/main/installation/packages/optimus_package.zip -o ./tmp/optimus_package.zip
	)

	@echo Unpack win python
	Powershell.exe Expand-Archive tmp\optimus_package.zip -DestinationPath tmp
...
:installPython_winpython
	@echo ================ INSTALL PYTHON ==========================
	Powershell.exe Set-ExecutionPolicy -ExecutionPolicy RemoteSigned
	if not exist tmp\python_installation.zip (
		...
		powershell wget https://github.com/ray-oh/Optimus-Installation/raw/main/installation/3rd_party_tools/python-3.10.9.amd64.zip -o ./tmp/python_installation.zip
	)
	if not exist installation\python-3.10.9.amd64 (
		@echo Unpack win python
		Powershell.exe Expand-Archive tmp\python_installation.zip -DestinationPath installation
	)
